...
<br>
<center style="font-weight: bolder; text-decoration: underline;">Liste des Notes de frais</center>
<br>
<table id="absences">
    <tr>
        <th>Nom & Prénoms</th>
    ...
    @foreach($notedefrais as $ntf)
        <tr>
            <td>{{ucfirst($ntf->user->name ?? '-')}} {{ucfirst($ntf->user->prenoms ?? '')}}</td>
            <td>{{ucfirst($ntf->projet ?? '-')}}</td>
            <td>{{ucfirst($ntf->montant ?? '_')}}</td>
            <td>{{ucfirst($ntf->created_at ?? '-')}}</td>
            <td>
                @switch($ntf->statut)
                      @case("0")
